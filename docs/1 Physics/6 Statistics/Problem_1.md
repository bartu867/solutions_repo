...
# ✅ Make plots appear inline (for Colab/Jupyter)
%matplotlib inline
...
# ✅ Define populations
populations = {
    "Uniform [0,1]": np.random.uniform(0, 1, N),                          # Variance ≈ 0.083
    "Exponential (λ=1)": np.random.exponential(1, N),                    # Variance = 1
    "Binomial (n=10, p=0.5)": np.random.binomial(10, 0.5, size=N),       # Variance = 2.5
}

# ✅ Add scaled exponential to increase variance
...
for row_idx, (dist_name, population) in enumerate(populations.items()):
    for col_idx, n in enumerate(sample_sizes):
        ...
        # Optional: overlay population mean
        ax.axvline(np.mean(population), color='red', linestyle='--', linewidth=1.5, label='True Mean')
        if col_idx == 0:
            ax.set_ylabel('Frequency')
        if row_idx == len(populations) - 1:
            ax.set_xlabel('Sample Mean')
...
plt.suptitle("CLT: Sampling Distributions by Population Shape and Variance", fontsize=20)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.show()
```
